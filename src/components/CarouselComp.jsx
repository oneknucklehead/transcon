...
const CarouselComp = () => {
  ...
  useEffect(() => {
    swiperRef?.pagination?.bullets.forEach((element) => {
      spans.push(<div className="bg-black w-10 h-10">{element}</div>);
    });
    console.log("spans: " + spans);
    console.log(swiperRef);
  }, [swiperRef]);
  return (
    <div className="grid gap-3 max-h-max justify-center items-center grid-cols-12">
      <button
        className="hidden md:flex col-span-1 bg-white w-[60px] p-4 rounded-full justify-center items-center"
      ...
      <button
        className="hidden col-span-1 bg-white w-[60px] p-4 rounded-full md:flex justify-center items-center"
        onClick={handleNext}
      >
        <svg
          width="27"
